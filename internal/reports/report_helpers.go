	...
	for _, p := range []struct {
		...
		if p.severity == errors && len(tableLevelIssues) != 0 {
			for _, issue := range tableLevelIssues {
				if issue == internal.RowLimitExceeded {
					toAppend := WarningClassified{
						IssueType:        IssueDB[internal.RowLimitExceeded].issueType,
		...
		if syntheticPK != nil {
			...
			if p.severity == warning {
				toAppend := WarningClassified{
					IssueType:        IssueDB[internal.MissingPrimaryKey].issueType, 
		...
		if p.severity == warning {
			for _, spFk := range conv.SpSchema[tableId].ForeignKeys {
				...
				if isChanged && srcFk.Name != spFk.Name {
					toAppend := WarningClassified{
						IssueType:        IssueDB[internal.IllegalName].issueType,
		...
		for _, colName := range colNames {
			...
			for _, i := range issues[colId] {
				...
				case internal.ForeignKey:
					toAppend := WarningClassified{
						IssueType:         IssueDB[i].issueType,
						Description: fmt.Sprintf("Column '%s' uses foreign keys which HarbourBridge does not support yet", spColName),
					}
					l = append(l, toAppend)
		...
		body = append(body, tableReportBody{Heading: heading, WarningBody: l})
	}
	return body
}

// Contains check string present in list.
func Contains(l []WarningClassified, str string) bool {
	for _, s := range l {
...
var IssueDB = map[internal.SchemaIssue]struct {
	...
	internal.DefaultValue:                {Brief: "Some columns have default values which HarbourBridge does not migrate. Please add the default constraints manually after the migration is complete", severity: note, batch: true, issueType: "MISSING_DEFAULT_VALUE_CONSTRAINTS"},
	internal.ForeignKey:                  {Brief: "Spanner does not support foreign keys", severity: warning, issueType: "FOREIGN_KEY_USES"},
	internal.MultiDimensionalArray:       {Brief: "Spanner doesn't support multi-dimensional arrays", severity: warning, issueType: "MULTI_DIMENSIONAL_ARRAY_USES"},
	internal.NoGoodType:                  {Brief: "No appropriate Spanner type", severity: warning, issueType: "INAPPROPIATE_TYPE"},
	internal.Numeric:                     {Brief: "Spanner does not support numeric. This type mapping could lose precision and is not recommended for production use", severity: warning, issueType: "NUMERIC_USES"},
	internal.NumericThatFits:             {Brief: "Spanner does not support numeric, but this type mapping preserves the numeric's specified precision", severity: suggestion, issueType: "NUMERIC_THAT_FITS"},
	internal.Decimal:                     {Brief: "Spanner does not support decimal. This type mapping could lose precision and is not recommended for production use", severity: warning, issueType: "DECIMAL_USES"},
	internal.DecimalThatFits:             {Brief: "Spanner does not support decimal, but this type mapping preserves the decimal's specified precision", severity: suggestion, issueType: "DECIMAL_THAT_FITS"},
	internal.Serial:                      {Brief: "Spanner does not support autoincrementing types", severity: warning, issueType: "AUTOINCREMENTING_TYPE_USES"},
	internal.AutoIncrement:               {Brief: "Spanner does not support auto_increment attribute", severity: warning, issueType: "AUTO_INCREMENT_ATTRIBUTE_USES"},
	internal.Timestamp:                   {Brief: "Spanner timestamp is closer to PostgreSQL timestamptz", severity: suggestion, batch: true, issueType: "TIMESTAMP_SUGGESTION"},
	internal.Datetime:                    {Brief: "Spanner timestamp is closer to MySQL timestamp", severity: warning, batch: true, issueType: "TIMESTAMP_WARNING"},
	internal.Time:                        {Brief: "Spanner does not support time/year types", severity: warning, batch: true, issueType: "TIME_YEAR_TYPE_USES"},
	internal.Widened:                     {Brief: "Some columns will consume more storage in Spanner", severity: warning, batch: true, issueType: "STORAGE_WARNING"},
	internal.StringOverflow:              {Brief: "String overflow issue might occur as maximum supported length in Spanner is 2621440", severity: warning, issueType: "STRING_OVERFLOW_WARNING"},
	internal.HotspotTimestamp:            {Brief: "Timestamp Hotspot Occured", severity: warning, issueType: "TIMESTAMP_HOTSPOT"},
	internal.HotspotAutoIncrement:        {Brief: "Autoincrement Hotspot Occured", severity: warning, issueType: "AUTOINCREMENT_HOTSPOT"},
	internal.InterleavedOrder:            {Brief: "can be converted as Interleaved with Table", severity: suggestion, issueType: "INTERLEAVE_TABLE_SUGGESTION"},
	internal.RedundantIndex:              {Brief: "Redundant Index", severity: warning, issueType: "REDUNDANT_INDEX"},
	internal.AutoIncrementIndex:          {Brief: "Auto increment column in Index can create a Hotspot", severity: warning, issueType: "AUTO-INCREMENT_INDEX"},
	internal.InterleaveIndex:             {Brief: "can be converted to an Interleave Index", severity: suggestion, issueType: "INTERLEAVE_INDEX_SUGGESTION"},
	internal.InterleavedNotInOrder:       {Brief: "if primary key order parameter is changed to 1 for the table", severity: suggestion, issueType: "INTERLEAVED_NOT_IN_ORDER"},
	internal.InterleavedAddColumn:        {Brief: "Candidate for Interleaved Table", severity: suggestion, issueType: "ADD_INTERLEAVED_COLUMN"},
	internal.IllegalName:                 {Brief: "Names must adhere to the spanner regular expression {a-z|A-Z}[{a-z|A-Z|0-9|_}+]", severity: warning, issueType: "ILLEAGAL_NAME"},
	internal.InterleavedRenameColumn:     {Brief: "Candidate for Interleaved Table", severity: suggestion, issueType: "RENAME_INTERLEAVED_COLUMN_PRIMARY_KEY"},
	internal.InterleavedChangeColumnSize: {Brief: "Candidate for Interleaved Table", severity: suggestion, issueType: "CHANGE_INTERLEAVED_COLUMN_SIZE"},
	internal.RowLimitExceeded:            {Brief: "Non key columns exceed the spanner limit of 1600 MB. Please modify the column sizes", severity: errors, issueType: "ROW_LIMIT_EXCEEDED"},
	internal.ShardIdColumnAdded:          {Brief: "column was added because this is a sharded migration and this column cannot be dropped", severity: note, issueType: "SHARD_ID_COLUMN_ADDED"},
	internal.ShardIdColumnPrimaryKey:     {Brief: "column is not a part of primary key. You may go to the Primary Key tab and add this column as a part of Primary Key", severity: suggestion, issueType: "SHARD_ID_ADD_COLUMN_PRIMARY_KEY"},
	internal.MissingPrimaryKey:           {issueType: "MISSING_PRIMARY_KEY"},
}
